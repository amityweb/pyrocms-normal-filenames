		...
		if ($folder)
		{
			...
			if (ci()->upload->do_upload($field))
			{
				...
				if ($data['type'] !== 'i')
				{
					...
					// we want PDFs to reference the actual filename like images! - AMITY Jan 2014
					//ci()->file_m->update($file_id, array('path' => '{{ url:site }}files/download/'.$file_id));
					ci()->file_m->update($file_id, array('path' => '{{ url:site }}uploads/{{ global:site_ref }}/files/'.$data['filename']));
				}
