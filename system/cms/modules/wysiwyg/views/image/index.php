		...
		<?php if ($current_folder): ?>
			...
			<table class="table-list" border="0" cellspacing="0">
				...
				<tbody>
					...
					$file_path = $image->location === 'local' ? 'files/thumb/'.$file_id.'/50/50' : 'files/cloud_thumb/'.$file_id;
					if(!is_file(FCPATH . UPLOAD_PATH.'files/'.$file_id))
					{
						$file_id = $image->id;
					}
					?>
					<tr class="<?php echo alternator('', 'alt') ?>">
                                                <td class="image"><img class="pyro-image" src="<?php echo base_url($image->location === 'local' ? 'files/thumb/'.$file_id.'/50/50' : 'files/cloud_thumb/'.$file_id) ?>" alt="<?php echo $image->alt_attribute ?>" width="50" onclick="javascript:insertImage('<?php echo $file_id."', '".htmlentities(addslashes($image->alt_attribute))."', '".$image->location."', '".$image->path ?>');" /></td>
